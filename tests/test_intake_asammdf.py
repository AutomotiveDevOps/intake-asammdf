...
from intake_asammdf import SQLSource
from intake_asammdf import SQLSourceAutoPartition
from intake_asammdf import SQLSourceManualPartition
...
def test_simple(temp_db):
    table, table_nopk, uri = temp_db
    d2 = SQLSource(uri, table, sql_kwargs=dict(index_col="p")).read()
    assert df.equals(d2)


def test_auto(temp_db):
    table, table_nopk, uri = temp_db
    s = SQLSourceAutoPartition(uri, table, index="p", sql_kwargs=dict(npartitions=2))
